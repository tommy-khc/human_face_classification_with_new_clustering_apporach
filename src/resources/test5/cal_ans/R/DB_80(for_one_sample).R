...
directory<-"/Users/Tomy_Chan/Documents/OneDrive - HKUST Connect/Yr4/Spring/MATH4983F/MATH4983F/Code/Code_for_ORL/Clustering_ORL/v2/test5(5sample)/cal_ans/R"
New_no_match_file<-file.path(directory,"New_no_match_v2_test5_80_5.txt")
New_no_match<-scan(file=New_no_match_file,sep = ",")
New_no_match<-matrix(New_no_match, nrow = 80, ncol = 80, dimnames = NULL)
#New_no_match<-read.table(file=New_no_match_file,header = FALSE,sep = ",")

clust_file<-file.path(directory,"clust_v2_test5_80_5.txt")
#clust<-read.table(file=clust_file,header = FALSE,sep = ",")
clust<-scan(file=clust_file, sep = ",")
clust1<-matrix(clust, nrow = 80, ncol = 80, byrow = TRUE, dimnames = NULL)
