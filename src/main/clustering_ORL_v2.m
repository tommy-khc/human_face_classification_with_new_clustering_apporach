...
for times = 1:4
    ...
    %txt for R
    formatSpec_txt = "New_no_match_v2_test4_%d.txt";
    str_txt = char(sprintf(formatSpec_txt,times));
    dlmwrite(str_txt,New_no_match,'precision','%10.50f');
    
    %clustering vadliation
    clust = zeros(size(New_no_match,1),100);
    for i=1:100
        clust(:,i) = kmedoids(New_no_match,i,'Algorithm','pam','OnlinePhase','off');
        pause(2);
    end
    
    %txt for R
    formatSpec_Clust_txt = "clust_v2_test4_%d.txt";
